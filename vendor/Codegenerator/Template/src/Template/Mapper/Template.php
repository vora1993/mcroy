...
class {$module} extends AbstractDbMapper implements {$module}Interface
{
    protected $tableName  = strtolower('{$module}');
    ...
    public function fetchCodeClient($code_client, $id=null) {
        $select = $this->getSelect();
        if($id) {
            $where = new Where;
        ...
        $select->where(array('code_client' => $code_client, 'status' => array(0, 1))); 
